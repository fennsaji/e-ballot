...
```bash
./target/release/e-ballot --dev
```

Purge the development chain's state:

```bash
./target/release/e-ballot purge-chain --dev
```

Start the development chain with detailed logging:

```bash
RUST_BACKTRACE=1 ./target/release/e-ballot -ldebug --dev
```

> Development chain means that the state of our chain will be in a tmp folder while the nodes are
> running. Also, **alice** account will be authority and sudo account as declared in the
> [genesis state](https://github.com/substrate-developer-hub/substrate-e-ballot/blob/main/node/src/chain_spec.rs#L49).
...
$ ./target/release/e-ballot --dev --base-path ./my-chain-state/
...
(`cargo build --release && ./target/release/e-ballot --dev --ws-external`)
by appending your own. A few useful ones are as follow.

```bash
# Run Substrate node without re-compiling
./scripts/docker_run.sh ./target/release/e-ballot --dev --ws-external

# Purge the local dev chain
./scripts/docker_run.sh ./target/release/e-ballot purge-chain --dev
